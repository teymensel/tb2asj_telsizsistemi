...
class EarthquakeService(QObject):
    ...
    def check_earthquakes(self) -> None:
        ...
        try:
            ...
            display_list = []
            for eq in earthquakes[:50]: # Son 50 depremi işle
                if self._passes_filter(eq):
    ...
    def _passes_filter(self, eq: Dict) -> bool:
        """Deprem filtreye uyuyor mu?"""
        # Şehir filtresi varsa kontrol et
        if self.city_filter:
            location = eq.get('title', '').lower()
            if self.city_filter not in location:
                return False
        return True

    def _process_new_events(self, earthquakes: List[Dict]):
        """Yeni ve kritik depremleri tespit et"""
        if self.last_check_time is None:
            self.last_check_time = datetime.now()
            # İlk çalıştırmada bildirim verme, sadece ID'leri kaydet
            for eq in earthquakes[:50]:
                self.known_earthquakes.append(self._generate_id(eq))
            return

        for eq in earthquakes:
            # 1. Filtre Kontrolü (Bildirim için de geçerli olsun)
            if not self._passes_filter(eq):
                continue
            ...
            # 3. Zaman Kontrolü (Çok eski depremleri okuma - 15 dk)
            try:
                # Kandilli formatı: "2024.02.15 20:10:05"
                eq_date_str = eq.get('date')
                eq_date = datetime.strptime(eq_date_str, "%Y.%m.%d %H:%M:%S")
                if datetime.now() - eq_date > timedelta(minutes=15):
                    continue # 15 dakikadan eskiyse atla
            except:
                pass # Tarih hatası varsa güvenli tarafta kal, devam et
    ...
    def get_announcement_text(self, eq: Dict) -> str:
        return (
            f"Deprem uyarısı! {eq['location']} bölgesinde "
