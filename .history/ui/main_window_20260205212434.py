...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        # İlk hava durumu anonsunu engellemek için bayrak
    ...
    def on_weather_updated(self, data: dict):
        ...
        print(f"[DEBUG] on_weather_updated çağrıldı. First check: {getattr(self, '_first_weather_check', 'YOK')}")
        self.weather_widget.update_weather(data)
        
        # İlk açılışta sesli okumayı engelle
        if getattr(self, '_first_weather_check', False):
            print("[DEBUG] İlk açılış olduğu için hava durumu anonsu ATLANDI.")
            self._first_weather_check = False
            return

        # Sesli oku (Sadece otomatik ayarı açıksa)
        # Not: Ayar anahtarını kontrol edelim: 'weather_hourly' kayıt ediliyor
        should_announce = settings.value("weather_hourly", True, type=bool)
        print(f"[DEBUG] Hava durumu anons yapılacak mı? {should_announce}")
    ...
    def show_about(self):
        ...
        QMessageBox.about(
            ...
            "<p>Version 1.2.1</p>"
            "<p>Powered by <b>Teymensel</b></p>"
            "<p><a href='https://teymensel.com'>teymensel.com</a></p>"
            "<ul>"
            "<li>Gelişmiş Bildirim Ayarları</li>"
            "<li>Deprem & Hava Durumu Entegrasyonu</li>"
            "</ul>"
        )
    ...
    def read_current_time(self):
        ...
        # Eğer bu bir saat başı bildirimi ise ve ayar açıksa,
        # 1 dakika sonra hava durmunu oku
        if now.minute == 0:
            is_hourly_weather = self.settings.value("weather_hourly", False, type=bool)
            if is_hourly_weather:
                # 60 saniye (1 dakika) sonra hava durumunu oku
                QTimer.singleShot(60000, self.read_weather)
