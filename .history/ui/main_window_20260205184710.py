...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        grid.addWidget(self.clock_widget, 0, 0, 1, 1)
        
        self.weather_widget = WeatherWidget()
        grid.addWidget(self.weather_widget, 1, 0, 2, 1)
        
        # Sağ sütun
        self.signal_meter = SignalMeterWidget()
        grid.addWidget(self.signal_meter, 0, 1, 1, 1)
        
        self.vox_control = VOXControlWidget()
        grid.addWidget(self.vox_control, 1, 1, 2, 1)
    ...
    def connect_signals(self):
        ...
        self.vox_control.ptt_pressed.connect(lambda: self.vox_controller.manual_ptt(True))
        self.vox_control.ptt_released.connect(lambda: self.vox_controller.manual_ptt(False))
    ...
    def load_settings(self):
        ...
        # Deprem servisi
        if settings.get('earthquake.enabled', True):
            min_mag = settings.get('earthquake.min_magnitude', 4.0)
            self.earthquake_service.set_min_magnitude(min_mag)
            
            interval = settings.get('earthquake.check_interval', 60)
            self.earthquake_service.set_check_interval(interval)
    ...
    def connect_radio(self):
        ...
        # COM port zorunluluğunu kaldırdık (AUX modu için) ama hala port seçilmesi gerekebilir
        # Eğer port "Otomatik" veya boşsa ve AUX modu seçiliyse hata vermemeli
            ...
            if ports:
                port = ports[0]
                 ...
                 QMessageBox.warning(
                    self,
                    "Uyarı",
                    "COM port bulunamadı veya ayarlanmamış."
                )
                 self.open_settings()
                 return
        ...
        if success or connection_type == 'AUX': 
            # AUX modunda port bağlantısı başarısız olsa bile ses devam etmeli
            # Ancak radio_connection.connect AUX için fake connection yapabilir.
            # Şimdilik radio_connection mantığını değiştirmedik, o yüzden AUX ise
            # hata olsa bile (COM açılmazsa) ses monitörünü açalım.
            
            # Hata olsa bile AUX modunda devam et, ancak UI'da uyarı verilebilir
            # Burada basitleştirip başarılı kabul edelim.
            pass
        ...
        # UI Güncelle (Connected state even if AUX)
    ...
    def on_error(self, error_msg: str):
        ...
        # Hata mesajlarını status bar'a yaz ama bağlantıyı kesme
        # self.signal_meter.set_status(error_msg, "error") 
    ...
    def show_about(self):
        ...
        QMessageBox.about(
            ...
            "<p>Version 1.2 (AUX Support)</p>"
            "<p>Gelişmiş telsiz yönetim ve bildirim sistemi</p>"
            "<p><b>Yenilikler:</b></p>"
            "<ul>"
            "<li>Canlı ses aktarımı (Audio Loopback)</li>"
            "<li>Manuel Bas-Konuş (PTT) butonu</li>"
            "<li>Geliştirilmiş VOX kontrolü</li>"
            "</ul>"
        )
