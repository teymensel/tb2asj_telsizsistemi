...
class EarthquakeWidget(QWidget):
    ...
    def update_list(self, earthquakes: list):
        ...
        for eq in earthquakes:
            ...
            # Gelen format: YYYY.MM.DD HH:MM:SS
            date_str = eq['date']
            try:
                # Daha okunabilir format: DD.MM HH:MM
                parts = date_str.split(' ')
                date_part = parts[0] # YYYY.MM.DD
                time_part = parts[1][:5] # HH:MM
                
                # Yıl kısmını atıp gün.ay yapalım, veya direkt kullanalım
                dt_obj = datetime.strptime(date_str, "%Y.%m.%d %H:%M:%S")
                time_val = dt_obj.strftime("%d.%m %H:%M")
            except:
                time_val = date_str[:16] # Hata olursa olduğu gibi al (kısaltarak)
