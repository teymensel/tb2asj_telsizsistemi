...
class SettingsDialog(QDialog):
    ...
    def create_weather_tab(self) -> QWidget:
        ...
        # API Anahtarı ve Test
        api_layout = QHBoxLayout()
        self.weather_api_key = QLineEdit()
        self.weather_api_key.setPlaceholderText("OpenWeatherMap API anahtarı")
        
        test_api_btn = QPushButton("Test Et")
        test_api_btn.setFixedWidth(80)
        test_api_btn.clicked.connect(self._test_weather_api)
        
        api_layout.addWidget(self.weather_api_key)
        api_layout.addWidget(test_api_btn)
        
        weather_layout.addRow("API Anahtarı:", api_layout)
        
        # Sonuç etiketi
        self.weather_test_result = QLabel("")
        self.weather_test_result.setStyleSheet("font-size: 9pt;")
        weather_layout.addRow("", self.weather_test_result)
        
        self.weather_city = QLineEdit("Istanbul") # Varsayılan şehir
        weather_layout.addRow("Şehir:", self.weather_city)
        
        self.weather_auto_announce = QCheckBox("Otomatik Duyuru (Her saat başı)")
        self.weather_auto_announce.setChecked(True)
        weather_layout.addRow("", self.weather_auto_announce)
        
        self.weather_hourly = QCheckBox("Saat Başından 1 Dk Sonra Oku")
        self.weather_hourly.setToolTip("Saat başı duyurusundan 1 dakika sonra hava durumunu okur")
        weather_layout.addRow("", self.weather_hourly)
    ...
    def create_general_tab(self) -> QWidget:
        widget = QWidget()
        layout = QVBoxLayout() # VBox'a çevirdik
        ...
        template_help = QLabel("Değişkenler: $saat, $tarih, $havadurumu, $sehir, $sicaklik, $sondeprem")
        template_help.setStyleSheet("font-size: 11px; color: #bdc3c7; font-weight: bold;") 
        template_layout.addWidget(template_help)
        
        self.announce_template_input = QTextEdit()
        self.announce_template_input.setPlaceholderText("Örn: Saat $saat. $sehir hava durumu: $havadurumu. Son deprem: $sondeprem")
        self.announce_template_input.setMaximumHeight(80) 
        template_layout.addWidget(self.announce_template_input)
        
        # Butonlar
        btn_layout = QHBoxLayout()
        
        reset_template_btn = QPushButton("Varsayılan")
        reset_template_btn.clicked.connect(self.reset_template)
        
        test_template_btn = QPushButton("🔊 Test Et")
        test_template_btn.clicked.connect(self._test_template_voice)
        
        btn_layout.addWidget(reset_template_btn)
        btn_layout.addWidget(test_template_btn)
    ...
    def _test_weather_api(self):
        """Hava durumu API anahtarını test et"""
        api_key = self.weather_api_key.text().strip()
        city = self.weather_city.text().strip() or "Istanbul"
        
        if not api_key:
            self.weather_test_result.setText("❌ Lütfen API anahtarı girin.")
            self.weather_test_result.setStyleSheet("color: red;")
            return

        self.weather_test_result.setText("⏳ Test ediliyor...")
        self.weather_test_result.setStyleSheet("color: orange;")
        self.weather_test_result.repaint() # UI güncelle
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.weather_test_result.setText(f"✅ Başarılı! {city}: {temp}°C, {desc}")
                self.weather_test_result.setStyleSheet("color: green;")
            elif response.status_code == 401:
                self.weather_test_result.setText("❌ Hata (401): Geçersiz API Anahtarı")
                self.weather_test_result.setStyleSheet("color: red;")
            elif response.status_code == 404:
                self.weather_test_result.setText(f"❌ Hata: Şehir bulunamadı ({city})")
                self.weather_test_result.setStyleSheet("color: red;")
            else:
                self.weather_test_result.setText(f"❌ Hata kodu: {response.status_code}")
                self.weather_test_result.setStyleSheet("color: red;")
                
        except Exception as e:
            self.weather_test_result.setText(f"❌ Bağlantı hatası: {str(e)[:50]}")
            self.weather_test_result.setStyleSheet("color: red;")
    ...
    def load_settings(self):
        ...
        if idx >= 0:
            self.eq_provider_combo.setCurrentIndex(idx)
    ...
    def save_settings(self):
        ...
        # weather_hourly burada kaydedilmeli ama önce UI elemanını eklemeliyim
        if hasattr(self, 'weather_hourly'):
            settings.set('weather.hourly', self.weather_hourly.isChecked())
    ...
    def reset_template(self):
        """Şablonu varsayılan haline getir"""
        default_template = "Saat $saat. $sehir hava durumu: $havadurumu."
        self.announce_template_input.setText(default_template)

    def _test_template_voice(self):
        """Şablonu test et"""
        if not self.notification_manager:
            return
            
        template = self.announce_template_input.toPlainText()
        if not template:
            template = "Şablon boş."
        ...
        current_date = f"{now.day} {months[now.month-1]} {days[now.weekday()]}"
        
        # Mock Hava Durumu
        full_weather = "15 derece, parçalı bulutlu"
        temp = "15"
        city = self.weather_city.text() or "Istanbul"
        
        # Mock Deprem
        last_eq = "Malatya Pütürge'de 3.5 büyüklüğünde"
        
        # Değişkenleri değiştir
        text = template.replace("$saat", current_time)
        text = text.replace("$tarih", current_date)
        text = text.replace("$havadurumu", full_weather)
        text = text.replace("$sicaklik", temp)
        text = text.replace("$sehir", city)
        text = text.replace("$sondeprem", last_eq)
        
        # Sadece hoparlöre okut
        self.notification_manager.send_notification(text, use_radio=False)
